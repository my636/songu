...
class HomeAction extends AdministratorAction {
	...
	public function invatecount()
	{
		...
		// 数据的格式化 与listKey保持一致
		$map = array();
		!empty($_POST['inviter_uid']) && $map['a.inviter_uid'] = array('in',explode(",", $_POST['inviter_uid']));
		!empty($_POST['receiver_uid']) && $map['a.receiver_uid'] = array('in',explode(",", $_POST['receiver_uid']));
