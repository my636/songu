...
class UserAction extends AdministratorAction {
	...
	private function _getUserOnlineList($limit, $map) {
		...
		// 数据格式化
		foreach($listData['data'] as $k => $v) {
			$listData['data'][$k]['uname'] = '<a href="'.U('admin/User/editUser',array('tabHash'=>'addUser','uid'=>$v['uid'])).'">'.$v['uname'].'</a> ('.$v['email'].')';
			$listData['data'][$k]['ctime'] = date('Y-m-d H:i:s',$v['ctime']);
			// 用户组数据
...
/*		if(!empty($_POST['_parent_dept_id'])) {
	...
	private function _getUserList($limit = 20, $map = array(), $type = 'index') {
		...
		foreach($listData['data'] as $k => $v) {
			// 获取用户身份信息
			$userCategory = model('UserCategory')->getRelatedUserInfo($v['uid']);
			$userCateString = '';
			$userCateArray = array();
			if(!empty($userCategory)) {
				$userCateString .= '<p>';
				foreach($userCategory as $value) {
					$userCateArray[] = '<span style="color:blue;cursor:auto;">'.$value['title'].'</span>';
				}
				$userCateString .= implode('&nbsp;', $userCateArray).'</p>';
			}
			//获取用户组信息
			...
			$listData['data'][$k]['uname'] = '<a href="'.U('admin/User/editUser',array('tabHash'=>'addUser','uid'=>$v['uid'])).'">'.$v['uname'].'</a>'.$userGroupIcon[$v['uid']].' ('.$v['email'].')'.$userCateString;
			$listData['data'][$k]['ctime'] = date('Y-m-d H:i:s',$v['ctime']);
			// 屏蔽部门信息，若要开启将下面的注释打开
/*			$department = model('Department')->getUserDepart($v['uid']);
			...
			switch(strtolower($type)) {
				...
				case 'dellist':
					...
					$this->opt['user_group'][$v['user_group_id']];
					// 操作数据
					$listData['data'][$k]['DOACTION'] = '<a href="'.U('admin/User/editUser',array('tabHash'=>'addUser','uid'=>$v['uid'])).'">'.L('PUBLIC_EDIT').'</a> - ';
	...
	public function addUser() {
        ...
        if($regInfo['register_audit'] == 0){
        	$this->pageKeyList = array('email','uname','password','sex','is_active','user_group','user_category');
        }
        ...
        $this->notEmpty = array('email','uname','password','user_category');
	...
	public function editUser(){
		...
		$this->_initUserListAdminMenu();
		// 列表key值 DOACTION表示操作
		// $this->pageKeyList = array('uid','email','uname','password','sex','identity','department_id','user_group','is_active');
		$this->pageKeyList = array('uid','email','uname','password','sex','user_group','user_category');
		$this->opt['type'] = array('2'=>L('PUBLIC_SYSTEM_FIELD'));
		// 字段选项配置
		$this->opt['sex'] = array('1'=>L('PUBLIC_MALE'),'2'=>L('PUBLIC_FEMALE'));
		//$this->opt['identity'] = array('1'=>L('PUBLIC_PERSONAL'),'2'=>L('PUBLIC_ORGANIZATION'));
...
/*		$depart = model('Department')->getUserDepart($uid);
		...
		$this->notEmpty = array('email','uname','user_category');
	...
	public function doUpdateUser() {
		...
		if($result || $r) {
			...
			// 保存用户组的信息
			$this->assign('jumpUrl', U('admin/User/editUser',array('uid'=>$uid,'tabHash'=>'addUser')));
	...
	public function doRebackUser() {
		...
		$result = model('User')->rebackUsers(intval($_POST['id']));
		if(!$result) {
			$return['status'] = 0;
			$return['data'] = L('PUBLIC_ADMIN_OPRETING_ERROR');				// 操作失败
		} else {
	...
	public function _initVerifyAdminMenu(){
		// tab选项
		$this->pageTab[] = array('title'=>'认证配置','tabHash'=>'config','url'=>U('admin/User/verifyConfig'));
		...
		$this->pageTab[] = array('title'=>'添加认证','tabHash'=>'addverify','url'=>U('admin/User/addVerify'));
		...
		foreach($listData['data'] as $k=>$v){
			...
			if($listData['data'][$k]['attach_id']){
			  ...
			  foreach($a as $key=>$val){
			  	if($val !== ""){		  		
			  		$listData['data'][$k]['attachment'] .= D('attach')->where("attach_id=$a[$key]")->getField('name').'&nbsp;<a href="'.U('widget/Upload/down',array('attach_id'=>$a[$key])).'" target="_blank">下载</a><br />';
			  	}
			  }
			  unset($a);
			}
	...
	public function verified() {
		...
		$this->pageKeyList = array('uname','usergroup_id','category','realname','idcard','phone','reason','info','attachment','DOACTION');
		$listData = D('user_verified')->where('verified=1 and usergroup_id!=6')->findpage(20);
		// 获取认证分类的Hash数组
		$categoryHash = model('CategoryTree')->setTable('user_verified_category')->getCategoryHash();
		foreach($listData['data'] as $k=>$v){
			$userinfo = model('user')->getUserInfo($listData['data'][$k]['uid']);
	...
	public function addVerify() {
        ...
        $this->notEmpty = array('uname','usergroup_id','company','realname','idcard','phone','reason','info','user_verified_category_id');
	...
	//	$data['attachment'] = t($_POST['attach']);
		...
		$data['verified_category_id'] = intval($_POST['user_verified_category_id']);
		$Regx1 = '/^[0-9]*$/';
        $Regx2 = '/^[A-Za-z0-9]*$/';
        $Regx3 = '/^[\x{4e00}-\x{9fa5}]+$/u';
	...
	public function editVerify(){
		...
		$this->pageKeyList = array('uid','uname','usergroup_id','verified_category_id','company','realname','idcard','phone','reason','info','attach');
		...
		$categoryHash = model('CategoryTree')->setTable('user_verified_category')->getCategoryHash();
		foreach($categoryHash as $key => $value) {
			$this->opt['verified_category_id'][$key] = $value;
		}
		// 认证组
		$auType = model('UserGroup')->where('is_authenticate=1')->select();
		foreach($auType as $k=>$v){
		...
		$this->notEmpty = array('usergroup_id','verified_category_id','company','realname','idcard','phone','reason','info','user_verified_category_id');
	...
	public function doEditVerify(){
        ...
        $data['info'] = t($_POST['info']);
        $data['attach_id'] = t($_POST['attach_ids']);
        $data['verified_category_id'] = intval($_POST['verified_category_id']);
	...
	public function category()
	{
		...
		$this->displayTree($treeData, 'user_category');
	}
	...
	public function officialCategory()
	{
		...
		$this->displayTree($treeData, 'user_official_category');
	}
	...
	{
		$this->pageTab[] = array('title'=>'官方用户配置','tabHash'=>'official','url'=>U('admin/User/official'));
		$this->pageTab[] = array('title'=>'官方用户分类','tabHash'=>'officialCategory','url'=>U('admin/User/officialCategory'));
		$this->pageTab[] = array('title'=>'官方用户列表','tabHash'=>'officialList','url'=>U('admin/User/officialList'));
		$this->pageTab[] = array('title'=>'添加官方用户','tabHash'=>'officialAddUser','url'=>U('admin/User/officialAddUser'));
	}
}