...
class TaskAction extends Action{
	...
	public function completeCustom(){
		...
		if ( $res ){
			...
			$button = '<span><input type="checkbox" id="taskfeed" value="1" class="checkbox"/>同时发表一条微博</span><a href="javascript:postfeed('.$id.');" class="btn-grey-white">确定</a>';
	...
	public function complete_task(){
		...
		if ( $res ){
			...
			//获得奖励
 			model('Task')->getReward( $reward->exp , $reward->score , $reward->medal->id );
			...
			if ( $type == 1 ){
				...
				//任务奖励
// 				if ( $level < 5 ){
					$button = '<span><input type="checkbox" id="taskfeed" value="1" checked="checked" class="checkbox"/>同时发表一条微博</span><a href="javascript:gonext('.$type.','.$level.')" class="btn-grey-white">确定</a>';
// 				}
	...
	public function complete_step(){
		...
		if ( $id ){
			...
			if ( $res ){
				$allcomplete = true;
				if ( $_REQUEST['tasktype'] == 2 ){
					$tasklevel = intval( $_REQUEST['tasklevel'] );
				...
				//获得奖励
				model('Task')->getReward( $reward->exp , $reward->score , $reward->medal->id , $GLOBALS['ts']['mid']);
				$res = array('allcomplete'=> $allcomplete , 'tasktype'=>$_REQUEST['tasktype'] ,'info'=>$info);
