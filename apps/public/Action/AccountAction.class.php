...
class AccountAction extends Action 
{
	...
	{
		$this->appCssList[] = 'public/account.css';
		...
		$this->assign($data);
		$this->setTitle(L('PUBLIC_ACCOUNT_INDEX'));			// 个人设置
		$this->display();
	}
    ...
    public function authenticate(){
		...
		// 获取认证分类信息
		if(!empty($verifyInfo['verified_category_id'])) {
			$verifyInfo['category']['title'] = D('user_verified_category')->where('user_verified_category_id='.$verifyInfo['verified_category_id'])->getField('title');
		}
		...
		// 获取认证分类
		$category = model('CategoryTree')->setTable('user_verified_category')->getCategoryList();
    ...
    public function doAuthenticate(){
        ...
        $data['verified_category_id'] = intval($_POST['verifiedCategory']);
        $Regx1 = '/^[0-9]*$/';
        $Regx2 = '/^[A-Za-z0-9]*$/';
        $Regx3 = '/^[\x{4e00}-\x{9fa5}]+$/u';
        if(empty($data['verified_category_id'])){
        	echo '请选择认证分类';exit;
        }
