	...
	<body>
		...
		<dl class="comment">
			...
			<dd>
				...
				<a href="javascript:void(0)" class="ico-details" event-node ='loadPost' event-args='feed_id={$feedid}&post_id={$sourceInfo['post_id']}' id="{$feedid}"><!--查看全文--></a>
				<div class="feed_img_lists" style="display:none;" id="post_{$feedid}_{$sourceInfo['post_id']}">
				</div>
				<php>}else{</php>
				<a href="{:U('weiba/Index/postDetail',array('post_id'=>$sourceInfo['post_id']))}" class="ico-details" target="_blank"></a>
				<php>}</php>
				</p>
			</dd>
		</dl>
