...
class WeibaReplyWidget extends Widget{
     ...
     //   echo $_POST['post_id'];exit;
        ...
        if($data['reply_id'] = D('weiba_reply')->add($data)){
            ...
            if($comment_id = D('comment')->add($datas)){
                ...
                $data1['storey'] = D('weiba_post')->where('post_id='.$data['post_id'])->getField('reply_all_count');
                ...
                D('feed')->where('feed_id='.$datas['row_id'])->setInc('comment_all_count');
                // 给应用UID添加一个未读的评论数
    ...
    public function delReply(){
    	...
    	if(!empty($reply_id)){
            ...
            model('Comment')->deleteComment($comment_id);
            return 1;
    	}
    	return false;
    }
