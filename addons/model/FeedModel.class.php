...
class FeedModel extends Model {
	...
	protected $fields = array('feed_id','uid','type','app','app_row_id','app_row_table','publish_time','is_del','from','comment_count','repost_count','comment_all_count','is_repost','_pk'=>'feed_id');
	...
	public function put($uid, $app = 'public', $type = '', $data = array(), $app_id = 0, $app_table = 'feed', $extUid = null, $lessUids = null, $isAtMe = true, $is_repost = 0) {
		...
		$data['from'] = isset($data['from']) ? intval($data['form']) : getVisitorClient();
		...
		if($feed_id && $feed_data) {
			...
			if($data['type'] == 'repost') {
				...
				$extUid[] = $data['sourceInfo']['uid'];						// 资源作者用户
	 ...
	 */
	public function doEditFeed($feed_id, $type, $title) {
	...
	public function searchFeed($key, $type, $loadId, $limit, $forApi = false,$feed_type) {
		switch($type){
			case 'following':
				...
				$where = !empty($loadId) ? " a.is_del = 0 AND a.feed_id <'{$loadId}'" : "a.is_del = 0";
