...
class UserModel	extends	Model {
	...
	protected $fields = array(
							0 =>'uid',1=>'login',2=>'password',3=>'login_salt',4=>'uname',5=>'email',
							6=>'sex',7=>'location',8=>'is_audit',9=>'is_active',10=>'is_init',11=>'ctime',12=>'identity',
							13=>'api_key',14=>'domain',15=>'province',16=>'city',17=>'area',18=>'reg_ip',
							19=>'lang',20=>'timezone',21=>'is_del',22=>'first_letter',23=>'intro',24=>'last_login_time',25=>'last_feed_id',26=>'last_post_time',27=>'search_key',28=>'invite_code','_autoinc'=>true,'_pk'=>'uid'
						);
		...
		if(isset($_POST)) {
			...
			if(!empty($_POST['ctime'])) {	
				if(!empty($_POST['ctime'][0]) && !empty($_POST['ctime'][1])) {
					// 时间区间条件
					$map['ctime'] = array('BETWEEN', array(strtotime($_POST['ctime'][0]), strtotime($_POST['ctime'][1])));
			...
			if(!empty($_POST['department'])) {
				$table .= " left join {$this->tablePrefix}user_department d on u.uid = d.uid and d.department_id = '".intval($_POST['department'])."'";
			}*/
			...
			if(!$_POST['uid']){
				...
				if(!empty($_POST['user_category'])) {
					...
					$category_uid = getSubByKey(D('user_category_link')->where('user_category_id='.intval($_POST['user_category']))->findAll(),'uid');
					if($group_uid){
						$map['uid'] = array('in', array_intersect($group_uid,$category_uid));
					}else{
						$map['uid'] = array('in', $category_uid);
					}
				}
			}
		}		
	...
	/**
	 * 添加用户
	 * @param array|object $user 新用户的相关信息|新用户对象
	 ...
	 * @param string $salt 密码附加参数，默认为11111
	...
	/**
	 * 改变用户的激活状态
	 * @param array $ids 用户UID数组
	 * @param integer $type 用户的激活状态，0表示未激活；1表示激活
	 ...
	 * 转移指定用户到指定部门
	 * @param array $uids 用户UID数组
	 * @param integer $user_department 部门ID
	...
	private function _getUserInfo($map , $field="*") {
		...
		if(!$user) {
    		...
    		foreach($userGroup[$uid] as $value) {
    			$groupIcon[] = '<img title="'.$value['user_group_name'].'" src="'.$value['user_group_icon_url'].'" style="width:auto;height:auto;display:inline;cursor:pointer;vertical-align:-5px;margin:0 0 0 -2px" />';
    		}
	 ...
	 * @param integer $uid 粉丝用户UID
	 * @param integer $mid 登录用户UID
	 ...
	 * @param integer $follow 关注状态值
	 * @param integer $limit 结果集数目，默认为100
	 * @param integer $max_id 主键最大值
	 * @param string $type 类型
	 * @param integer $noself 搜索结果是否包含登录用户，默认为0
	 * @return array 用户列表数据
	 */
	public function searchUser($key = '', $follow = 0, $limit = 100, $max_id = '', $type = '', $noself = '0', $page , $atme)
	{
		...
		foreach($list['data'] as &$v) {
			$v = $this->getUserInfoForSearch($v['uid'] , 'uid,uname,sex,domain,search_key');
		}
