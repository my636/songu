...
class MessageModel extends Model {
    ...
    public function getDetailList($map,$limit=20,$order = 'a.message_id DESC') {
        $field = 'a.*,b.from_uid AS fuid,b.type,b.min_max ';
        $table = $this->tablePrefix.'message_content AS a LEFT JOIN '.$this->tablePrefix.'message_list AS b ON a.list_id = b.list_id';
    ...
    public function setMessageIsRead($list_ids = null, $member_uid) {
        ...
        return false !== D('message_member')->where($map)->setField('new', 0);
    }
    ...
    private function _addMessageMember($data, $from_uid) {
        ...
        foreach($data['member'] as $k => $m) {
            ...
            if(!empty($memberInfo)) {
                ...
                $member['new'] = $m == $from_uid ? $memberInfo['new'] : $memberInfo['new'] + 1;
