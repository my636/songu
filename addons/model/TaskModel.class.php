...
class TaskModel extends Model{
	...
	public function _executeTask( $excutetype , $num , $uid , $type , $userdata){
		...
		if ( $type == 1){
			...
			switch ( $excutetype ){
				case 'weibopost':
					$rescount = model('Feed')->where("uid=".$uid." and type='post' and publish_time>".$starttime)->limit($num)->count();
					break;
				case 'weiborepost':
			...
			switch ( $excutetype ){
					//原创微博
				case 'weibopost':
					$rescount = model('Feed')->where('uid='.$uid." and type='post'")->limit($num)->count();
					break;
					//转发微博
				case 'weiborepost':
				...
				case 'weibocomment':
					...
					//上传头像
				case 'uploadface':
// 					return file_exists(UPLOAD_PATH.'/avatar'.model('Avatar')->convertUidToPath($uid).'/original.jpg');
					return true;
					break;
					//粉丝数
				case 'following':
