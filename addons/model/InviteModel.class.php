...
class InviteModel extends Model
{
	...
	public function getInviteUserList($uid, $type, $isAdmin = false)
	{
		...
		$map['u.is_init'] = 1;
		$list = D()->table('`ts_invite_code` AS c LEFT JOIN `ts_user` AS u ON c.receiver_uid = u.uid')
				   ->field('c.*')
				   ->where($map)
	...
	public function doInvite($email, $detail, $uid, $isAdmin = false)
	{
		...
		$config['face'] = $userInfo['avatar_middle'];
	...
	public function applyInviteCode($uid, $type)
	{
		...
		$applyCredit = abs($creditRule['core']['code']['gold']);
		// 更新积分
		$userCredit = model('Credit')->getUserCredit($uid);
		$gold = $userCredit['gold'];
		if($gold < $applyCredit) {
			return false;
		} else {
			// 添加邀请码操作
			...
			if($result || $type == 'email') {
				model('Credit')->updateUserCredit($uid, 'core_code');
				return true;
			} else {
				return false;
			}
