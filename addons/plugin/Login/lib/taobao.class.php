...
class taobao{
	...
	function getUrl($redirect_uri){
		...
		$this->loginUrl = 'https://oauth.taobao.com/authorize?'.'client_id='.TAOBAO_KEY.'&redirect_uri='.urlencode($redirect_uri).'&response_type=code';
	...
	function userInfo(){
		if($_SESSION['taobao']['uid']){
			...
			$user['uname']      = $_SESSION['taobao']['uname'];
			$user['province']   = 0;
			$user['city']       = 0;
			$user['location']   = '';
	...
	function checkUser(){
		if($_GET['code']){
			...
			$field = 'grant_type=authorization_code&client_id='.TAOBAO_KEY.'&code='.$_GET['code'].'&client_secret='.TAOBAO_SECRET.'&redirect_uri='.urlencode($redirect_uri);
			$ch = curl_init();
