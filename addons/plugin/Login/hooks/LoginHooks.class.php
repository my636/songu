...
class LoginHooks extends Hooks {
    ...
    //可同步发布动态的站点
    private static $validPublish = array("sina", "qq", "qzone", "renren", "douban");
    //应用名称
    ...
    public function weibo_publish_after($param) {
        ...
        if($data['type']=='postimage'){
            $attach = M('attach')->find($data['attach'][0]);
        ...
        //当前操作如果是登录
            ...
            if ( $info = D('Login')->where ( "`type_uid`='" . $userinfo ['id'] . "' AND type='{$type}'" )->find () ) {
                ...
                //已经绑定过，执行登录操作，设置token
                    ...
                    $result ['status'] = 1;
                    $result ['url'] = U('public/Index/index');
                    $result ['info'] = "同步登录成功";
                    return;
                }
            }
    ...
    public function login_input_footer($param) {
        ...
        if ($regInfo ['register_type'] != 'admin' && ! empty ( $platform )) {
            ...
            $html .= '<span>其它帐号登录：</span>';
            $html .= '<dl>';
            foreach ( $platform as $key => $value ) {
                $html .= sprintf ( "<dd><a href=\"%s\"><i class=\"ico-%s\"></i></a></dd>", $value, $key );
            }
            $html .= '</dl>';
            $html .= '</div>';
            echo $html;
