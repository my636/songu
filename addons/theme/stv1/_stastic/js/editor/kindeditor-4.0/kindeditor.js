...
K.options = {
	...
	htmlTags : {
		...
		a : ['href', 'target', 'name'],
		'p,span,font,div' : [
			'align', '.text-align', '.color' , '.font-weight' , '.text-decoration' ,'.text-indent','.font-style'
		],
...
KEditor.prototype = {
	...
	create : function() {
		...
		statusbar.removeClass('statusbar').addClass('ke-statusbar')
			.append('<span class="ke-inline-block ke-statusbar-center-icon">'+self.lang('caninputcount')+'</span>')
...
KISSY.Editor = function(id , mode , focusset){
	var editor,modeitem = [];
	focusset = (focusset===false) ? false : true;
	mode     = (mode=='photo') ? 'photo' : 'simple';
	modeitem['photo']   = [ 'bold', 'italic', 'underline',  'forecolor','removeformat', '|', 'justifyleft', 'justifycenter', 'justifyright','emoticons','image'];
	//modeitem['simple'] = [ 'bold', 'italic', 'underline',  'forecolor','removeformat', '|', 'justifyleft', 'justifycenter', 'justifyright','emoticons'];
	modeitem['simple'] = [ 'bold', 'italic', 'underline',  'forecolor','removeformat'];
	KindEditor.ready(function(K) {
		editor = K.create('textarea[name="'+id+'"]', {
			items : modeitem[mode],
			filterMode : true,
			afterChange : function() {
				K('span#t_count').html( 20000 - this.count() );
			},
			afterCreate : function() {
				if(  focusset==true ){  
					   editor.focus(id);
				...
				K.ctrl(editor.edit.doc, 13, function() {
						
						editor.PASSUNLOAD = true;
						$("#"+id).parents('form').submit();
				});

				
				$("#"+id).parents('form').submit(function(txt){
					editor.sync();
					try{
						if( eval( $(this).attr('onbeforesubmit') ) || $(this).attr('onbeforesubmit')==undefined ){
							editor.PASSUNLOAD = true;
							return true;
						}else{
							editor.PASSUNLOAD = false;
							return false;
						}
					}catch(e){
						alert('onbeforesubmit方法定义有误');
						return false;
					}
				});
			}
