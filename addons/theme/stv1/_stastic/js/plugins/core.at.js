...
core.at = {
	...
	insertAt: function(textarea) {
		...
		var html = '<div class="talkPop alL" id="atchoose" style="*padding-top:20px;" event-node="atdiv">'
			 ...
			 + '<div class="faces_box" id="at_content"><img src="'+ THEME_URL+'/image/load.gif" class="alM"><img src="'+ THEME_URL+'/image/load.gif" class="alM"></div></div></div></div>';
		
		//$(this.parentDiv).append(html);
		$('body').append(html);
		var pos = $(this.objat).offset();
		
		$('#atchoose').css({top:(pos.top+5)+"px",left:(pos.left-5)+"px","z-index":9999});
		
		
		$.post(U('public/Feed/getFriendGroup') , {} , function(res){
			$('#at_content').html(res);
		});
